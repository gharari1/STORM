...
with xr.open_dataset(os.path.join("../Datasets/", config.train_data)) as ds:
    arrTrainData = ds['log_precipitation']
...
with xr.open_dataset(os.path.join("../Datasets/", config.val_data)) as ds:
    arrValData = ds['log_precipitation']
...
model.load_weights(os.path.join("../saved_weights/", config.model_weights))
print(f"Loaded weights from {config.model_weights}")

print("\n--- Freezing layers for fine-tuning the bottleneck ---")
layers_to_freeze = ['time_distributed', 'time_distributed_1', 'time_distributed_2',
                    'time_distributed_3', 'time_distributed_4', 'time_distributed_5',
                    'time_distributed_6','conv_lstm2d', 'batch_normalization',
                    'dropout_1','up_sampling2d', 'concatenate', 'conv2d_4', 'conv2d_5']
